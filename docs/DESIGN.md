...
大量保有府令は1日40〜70件出るため全件通知はノイズになる。既定は「新規の5%超え」のみ:
...
- 実測ではこの絞り込みで1日40件超 → 数件程度になる(2026-07-08実データ: 43件中6件)
- 変更報告書(買い増し・売り抜けの動き)も通知したくなったら `change_report` をルールに足すだけ。
  その場合は `ratio > prev_ratio`(買い増しのみ)のような数値条件の追加実装を検討する
